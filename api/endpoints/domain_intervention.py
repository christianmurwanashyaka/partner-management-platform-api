...
from schemas.domain_intervention import (
    DomainInterventionRead,
    DomainInterventionList,
    DomainInterventionCreate,
)
...
@router.post(
    "/", response_model=DomainInterventionRead, dependencies=[Depends(admin_access)]
)
async def create_domain_intervention(
    request: Request,
    domain_intervention: DomainInterventionCreate,
    ...
    if await check_if_exists(DomainIntervention, db, name=domain_intervention.name):
        raise HTTPException(
            ...
            detail="Domain intervention with this name already exists",
        )

    new_domain_intervention = DomainIntervention(
        name=domain_intervention.name,
        description=domain_intervention.description,
        created_by=user,
    )
...
async def get_domain_interventions(
...
async def get_domain_intervention(uuid: str, db: AsyncSession = Depends(get_db)):
    ...
    if not domain_intervention:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, detail="Domain intervention not found"
        )
...
@router.delete(
    "/{uuid}",
    response_model=DomainInterventionRead,
    dependencies=[Depends(admin_access)],
)
async def delete_domain_intervention(
