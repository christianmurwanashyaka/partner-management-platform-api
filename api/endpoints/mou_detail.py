...
async def create_mou_detail(
    ...
    try:
        ...
        new_mou_detail = MouDetail(project_id=project_id, created_by=user)
        ...
        await upload_document(
            memo_describing_the_source_of_funds,
            DocumentType.MEMO_DESCRIBING_THE_SOURCE_OF_FUNDS,
        )
        await upload_document(
            capacity_building_transfer_plan,
            DocumentType.CAPACITY_BUILDING_TRANSFER_PLAN,
        )
        await upload_document(
            memo_describing_the_long_term_objective,
            DocumentType.MEMO_DESCRIBING_THE_LONG_TERM_OBJECTIVES,
        )
        ...
        if mou_detail_to_return is None:
            raise HTTPException(
                ...
                detail="Mou Detail not loaded",
            )
        if mou_detail_to_return.project is None:
            project = await db.execute(
                select(Project).where(Project.uuid == mou_detail_to_return.project_id)
            )
...
@router.patch(
    "/{uuid}", response_model=MouDetailRead, dependencies=[Depends(partner_access)]
)
async def update_mou_detail(
    ...
    try:
        ...
        if (
            mou_detail.created_by != current_user.email
            and current_user.role != UserRole.ADMIN
        ):
            raise HTTPException(
                status.HTTP_403_FORBIDDEN,
                detail="Not authorized to update this MouDetail",
            )
        ...
        if party_ids is not None:
            ...
            parties = await db.execute(
                select(Party).where(Party.uuid.in_(party_ids_list))
            )
            parties = parties.scalars().all()
            for party in parties:
...
async def get_mou_detail(
    ...
    try:
        ...
        if not mou_detail:
            raise HTTPException(
                status.HTTP_404_NOT_FOUND, detail="MOU detail not found"
            )
        ...
        if current_user.role in [
            ...
            and mou_detail.created_by == current_user.email
        ):
            return mou_detail
        else:
            raise HTTPException(
                status.HTTP_403_FORBIDDEN,
                detail="You are not authorized to access this MOU detail",
            )
