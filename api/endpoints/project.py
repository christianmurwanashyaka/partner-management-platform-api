...
from helpers.db import (
    load_project_related_objects,
)
from notification.handlers import EmailNotificationHandler
from schemas.activity import ProjectActivityResponse
...
async def create_project(
    request: Request, project: ProjectCreate, db: AsyncSession = Depends(get_db)
):
    ...
    if (
        project.total_budget is not None
        and abs(project.total_budget - calculated_total_budget) > 0.01
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    new_project = Project(
        ...
        fiscal_year_budgets=[
            {"fiscal_year": item.fiscal_year, "budget": item.budget}
            for item in project.fiscal_year_budgets
        ],
...
@router.patch(
    "/{uuid}", response_model=ProjectRead, dependencies=[Depends(partner_access)]
)
    ...
    try:
        ...
        if "fiscal_year_budgets" in update_data:
            ...
            calculated_total = sum(
                fby["budget"] for fby in update_data["fiscal_year_budgets"]
            )
        ...
        await update_related_mou_application(
            project, db, email_handler, background_tasks
        )
...
async def get_projects(
    ...
    page_size: int = 100,
    approved: bool = Query(
        False, description="Filter projects with approved MOU applications only"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    try:
        ...
        if approved:
            ...
            query = query.join(
                MouApplication, MouApplication.mou_detail_id == MouDetail.uuid
            )
...
async def get_project_activities(
    ...
    if not activities_list:
        raise HTTPException(
            ...
            detail="No activities found for this project",
        )

    total_items_query = (
        select(func.count()).select_from(Activity).where(Activity.project_id == uuid)
    )
    ...
    try:
        query = (
            ...
            .options(
                ...
                joinedload(
                    Project.activities
                ),  # Add any other related models as needed
            )
        )
        ...
        if current_user.role in ["admin", "moh_staff"]:
            ...
            and project.organization.created_by == current_user.email
        ):
            await load_project_related_objects(db, project, load_goals=True)
            return project
        else:
