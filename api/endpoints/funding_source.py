...
@router.post('/', response_model=FundingSourceRead, dependencies=[Depends(admin_access)])
async def create_funding_source(request: Request, funding_source: FundingSourceCreate, db: AsyncSession = Depends(get_db)):
    user = request.state.user.email

    if await check_if_exists(FundingSource, db, name=funding_source.name):
        raise HTTPException(status.HTTP_409_CONFLICT, detail='Funding source with this name already exists')

    funding_source = FundingSource(name=funding_source.name, description=funding_source.description, created_by=user)
...
@router.get('/', response_model=PaginatedResponse[FundingSource], dependencies=[Depends(admin_access)])
...
@router.get('/{uuid}', response_model=FundingSourceRead, dependencies=[Depends(admin_access)])
