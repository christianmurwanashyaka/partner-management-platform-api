...
class MouReview(CommonBaseModel, table=True):
    ...
    current_reviewer: 'User' = Relationship(sa_relationship_kwargs={'lazy': 'noload'})
    mou_application_id: uuid.UUID = Field(foreign_key='mou_application.uuid', index=True)
    mou_application: 'MouApplication' = Relationship(back_populates='reviews', sa_relationship_kwargs={'lazy': 'noload'})
    comments: List['MouComment'] = Relationship(back_populates='mou_review', sa_relationship_kwargs={'lazy': 'noload'})
