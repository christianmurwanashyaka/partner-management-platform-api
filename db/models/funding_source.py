...
class FundingSource(CommonBaseModel, table=True):
    ...
    name: str = Field(..., description="Name of the funding source", index=True)
    description: str | None = Field(default=None, nullable=True, description="Optional description of the funding source")
    projects: List['Project'] = Relationship(back_populates='funding_source', sa_relationship_kwargs={'lazy': 'noload'})
