...
class MouComment(CommonBaseModel, table=True):
    ...
    mou_application: 'MouApplication' = Relationship(back_populates='comments', sa_relationship_kwargs={'lazy': 'noload'})

    mou_approval_id: Optional[uuid.UUID] = Field(foreign_key='mou_approval.uuid', nullable=True, index=True)
    mou_approval: Optional['MouApproval'] = Relationship(back_populates='comments', sa_relationship_kwargs={'lazy': 'noload'})

    mou_review_id: Optional[uuid.UUID] = Field(foreign_key='mou_review.uuid', nullable=True, index=True)
    mou_review: Optional['MouReview'] = Relationship(back_populates='comments', sa_relationship_kwargs={'lazy': 'noload'})

    mou_approval_or_review_id: Optional[uuid.UUID] = Field(foreign_key='mou_approval_or_review.uuid', nullable=True, index=True)
    mou_approval_or_review: Optional['MouApprovalOrReview'] = Relationship(back_populates='comments', sa_relationship_kwargs={'lazy': 'noload'})
