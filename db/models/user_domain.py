...
class UserDomain(CommonBaseModel, table=True):
    ...
    user: 'User' = Relationship(back_populates='domains', sa_relationship_kwargs={'lazy': 'noload'})

    domain_id: uuid.UUID = Field(default=uuid.UUID, foreign_key='domain_intervention.uuid', index=True)
    domain_intervention: 'DomainIntervention' = Relationship(back_populates='users', sa_relationship_kwargs={'lazy': 'noload'})
