...
async def generate_mou_doc(
    ...
    for party in parties:
        if party.organization_id:
            responsibilities_str = "\n".join(
                [
                    f"{idx + 1}. {responsibility}"
                    for idx, responsibility in enumerate(party.responsibilities)
                ]
            )
...
async def generate_mou_action_plan(mou_application, db: AsyncSession = Depends(get_db)):
    ...
    query = (
        ...
        .options(selectinload(MouApplication.mou_detail))
    ...
    if not mou_application:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, detail="MouApplication not found"
        )

    funding_source_query = select(FundingSource).where(
        FundingSource.uuid == project.funding_source_id
    )
    funding_source_result = (
        await db.execute(funding_source_query)
    ).scalar_one_or_none()
    funding_source = (
        funding_source_result
        if funding_source_result
        else (project.other_funding_source if project.other_funding_source else "N/A")
    )

    budget_type_query = select(BudgetType).where(
        BudgetType.uuid == project.budget_type_id
    )
    budget_type = (await db.execute(budget_type_query)).scalar_one_or_none()

    funding_unit_query = select(FundingUnit).where(
        FundingUnit.uuid == project.funding_unit_id
    )
    ...
    organization_query = select(Organization).where(
        Organization.uuid == project.organization_id
    )
    ...
    project_activities_query = select(Activity).where(
    ...
    for activity in activities:
        ...
        for input_detail in input_details:
            ...
            for domain in activity_domains:
                domain_name = getattr(
                    getattr(domain, "domain_intervention", None), "name", "N/A"
                )
                sub_domain_name = getattr(
                    getattr(domain, "sub_domain", None), "name", "N/A"
                )
                sub_domain_function_name = getattr(
                    getattr(domain, "sub_domain_function", None), "name", "N/A"
                )
                sub_function_name = getattr(
                    getattr(domain, "sub_function", None), "name", "N/A"
                )
...
async def generate_implementation_plan(report: Report, db: AsyncSession):
    ...
    headers = [
        ...
        "Organization Type",
        "Financing Schemes",
        "Sub Financing Schemes",
        "Financing Agents",
        "Sub Financing Agents",
    ...
    organization_query = (
        select(Organization)
        .where(Organization.uuid == project.organization_id)
        .options(
    ...
    # Helper function to join names with commas
    def join_names(items):
        return ", ".join(item.name for item in items) if items else "N/A"

    # Get schemes data
    financing_schemes = join_names(organization.financing_schemes)
    sub_financing_schemes = join_names(organization.sub_financing_schemes)
    financing_agents = join_names(organization.financing_agents)
    sub_financing_agents = join_names(organization.sub_financing_agents)
    health_care_providers = join_names(organization.health_care_providers)
    sub_health_care_providers = join_names(organization.sub_health_care_providers)
    ...
    funding_source_name = (
        funding_source.name if funding_source else project.other_funding_source or "N/A"
    )
    ...
    for activity in activities:
        ...
        report_activity_query = select(ReportActivity).where(
            ReportActivity.activity_uuid == activity.uuid
        )
        ...
        for input_detail in input_details:
            ...
            for domain in activity_domains:
                ...
                data = [
                    ...
                    (
                        ", ".join(report_activity.accomplishments)
                        if report_activity and report_activity.accomplishments
                        else "N/A"
                    ),
                ]
                ws.append(data)
